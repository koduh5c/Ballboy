...
public class BlockedBackground implements BackgroundDrawer {
    ...
    public void draw(
        ...
        Text winnerMessage = model.levelManager().buildWinnerText(width, height);
        Text scoreboard = model.levelManager().buildScoreboardText(width, height);

        pane.getChildren().addAll(sky, floor, winnerMessage, scoreboard);
    }
